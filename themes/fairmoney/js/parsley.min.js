	...
	validators: {
		string: {
			fn: function (value, requirements) {
				var filteredValue = value.trim();

				var pattern = /^[A-Za-z0-9 '\\/-]*$/;
				var regexp = new RegExp(pattern);

				return regexp.test(filteredValue);
			},
			priority: 31
		},
		firstletter: {
		...
		money: {
			fn: function (value, requirements) {
				var minValue = requirements[0];
				var maxValue = requirements[1];

				var filteredValue = value.replace(/([\s|£|,])/g, ''); 

				return (!isNaN(parseInt(filteredValue)) && filteredValue >= minValue && filteredValue <= maxValue);
			},
			priority: 32
		},
		period: {
			fn: function (value, requirements) {
				var minPeriod = requirements[0];
				var maxPeriod = requirements[1];

				var filteredValue = value.trim(); 

				return (!isNaN(parseInt(filteredValue)) && filteredValue >= minPeriod && filteredValue <= maxPeriod);
			},
			priority: 32
		},
		date: {
			fn: function (value, requirements) {
				var elements = requirements.split(',');

				var monthElement = jQuery(elements[0].trim());
				var yearElement = jQuery(elements[1].trim());

				var monthValue = jQuery(monthElement).val();
				var yearValue = jQuery(yearElement).val();

				if (monthValue == '' || yearValue == '') {
					return true;
				}

				var dateString =  yearValue + '-' + monthValue + '-' + value;
		...
		postcode: {
			fn: function (value, requirements) {
				...
				var pattern = /^(GIR 0AA|[A-PR-UWYZ]([0-9][0-9A-HJKPS-UW]?|[A-HK-Y][0-9][0-9ABEHMNPRV-Y]?)[0-9][ABD-HJLNP-UW-Z]{2})$/i;
				var regexp = new RegExp(pattern);
